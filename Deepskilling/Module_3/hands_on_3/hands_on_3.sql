...
-- Courses where every enrolled student received grade A
...
WHERE p.salary =
(
    SELECT MAX(salary) FROM professors p2 WHERE p2.department_id = p.department_id
);

-- Departments whose average professor salary > 85000
...
SELECT s.student_id, s.first_name || ' ' || s.last_name AS student_name,d.dept_name, COUNT(e.course_id) AS total_courses,
ROUND
(
AVG
(
...
FROM courses c LEFT JOIN enrollments e ON c.course_id=e.course_id GROUP BY c.course_name,c.course_code;
...
-- Verify
SELECT * FROM enrollments WHERE student_id = 12 AND course_id = 1;